...
CREATE TABLE users (
    user_id INT AUTO_INCREMENT PRIMARY KEY,           -- Egyedi azonosító
    name VARCHAR(100) NOT NULL,                       -- Felhasználónév
    email VARCHAR(255) NOT NULL UNIQUE,               -- Egyedi e-mail cím
    password VARCHAR(255) NOT NULL,                   -- Jelszó 
    role ENUM('admin', 'user') DEFAULT 'user',        -- Jogosultsági szint (alapértelmezett: user)
